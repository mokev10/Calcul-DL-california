...
# Intègre validation AAMVA optionnelle via aamva_utils.py (si présent)
...
# -------------------------
# ZIP_DB minimal fallback (can be extended by fetching)
...
# -------------------------
# Simple SVG parsing & rasterization helpers (rect + simple path)
...
# Session defaults
defaults = {
    "ln_input": "HARMS", "fn_input": "ROSA", "address1_input": "2570 24TH STREET", "address2_input": "",
    "zip_select": next(iter(ZIP_DB.keys())) if ZIP_DB else "90001", "city_select": ZIP_DB.get(next(iter(ZIP_DB.keys())), {}).get("city", ""),
    "state_input": "CA", "office_select": "", "sex_input": "M", "dob_input": datetime.date(1990,1,1),
    "h1_input": 5, "h2_input": 10, "w_input": 160, "eyes_input": "BRN", "hair_input": "BRN",
    "cls_input": "C", "rstr_input": "NONE", "endorse_input": "NONE", "iss_input": datetime.date.today()
}
...
with col_zip:
    zip_select = st.selectbox("Code postal", options=zip_options, index=zip_options.index(st.session_state.get("zip_select")) if st.session_state.get("zip_select") in zip_options else 0, key="zip_select")
with col_city:
    city_select = st.selectbox("Ville", options=city_options, index=city_options.index(st.session_state.get("city_select")) if st.session_state.get("city_select") in city_options else 0, key="city_select")

office_all = sorted(set(FIELD_OFFICE_MAP.values()))
office_select = st.selectbox("Field Office", options=office_all, index=0 if not st.session_state.get("office_select") else office_all.index(st.session_state.get("office_select")) if st.session_state.get("office_select") in office_all else 0, key="office_select")
...
if generate:
    ...
    city_val = st.session_state["city_select"]; state_val = st.session_state["state_input"]
    ...
    fields = {
        ...
        "DAQ": dl, "DAG": address1_val.upper(), "DAH": address2_val.upper() if address2_val else None,
        "DAI": city_val.upper(), "DAJ": state_val.upper(), "DAK": re.sub(r"\D","",postal_val)[:10],
    ...
    # Generate PDF417 (SVG) if available
    st.subheader("PDF417")
    ...
    # Final PDF card (if reportlab available)
    if _REPORTLAB_AVAILABLE:
        try:
            pdf_bytes = create_pdf_bytes({
                "Nom": ln_val, "Prénom": fn_val, "Sexe": st.session_state.get("sex_input"),
                "DOB": fields.get("DBB"), "Adresse": address1_val, "Ville": city_val, "État": state_val, "Code postal": postal_val,
