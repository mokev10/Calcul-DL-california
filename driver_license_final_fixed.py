#!/usr/bin/env python3
# driver_license_final_final.py
# Version finale corrigée — thème (dark/light) robuste, injection CSS sûre (f-strings), pas de .format() sur CSS
# Collez ce fichier en remplacement complet de votre script existant.
...
# -------------------------
# THEME MODULE (State-aware, Atomic UI)
# -------------------------
# Initialize theme state (default 'dark' as requested)
...
def _attempt_rerun():
    ...
    If neither is available, do nothing (CSS injection still applies).
    """
    try:
    ...
    try:
        ...
        # Environment doesn't allow rerun; rely on CSS injection and subsequent interactions.
        return

def toggle_theme_callback():
    """Flip theme and trigger rerun for full re-render."""
    st.session_state["theme"] = "light" if st.session_state["theme"] == "dark" else "dark"
    inject_theme_css()  # re-inject immediately
    _attempt_rerun()

def inject_theme_css():
    """
    Inject CSS using f-strings and double-braced CSS blocks to avoid Python .format() KeyError.
    Uses st.markdown(unsafe_allow_html=True) as requested.
    Must be called before rendering widgets for best effect.
    """
    ...
    # Use f-string and double braces {{ }} to include literal braces in CSS safely.
    theme_css = f"""
    <style>
    :root {{
      --bg-color: {colors['bg']};
    ...
    /* Apply to Streamlit app container only (avoid global html/body overrides) */
    ...
    """
    # Inject via st.markdown as requested (unsafe_allow_html=True)
...
# -------------------------
# Application data & helpers (unchanged logic)
# -------------------------
...
# -------------------------
# Header with theme toggle (atomic layout)
# -------------------------
...
with right_col:
    # Show icon for the mode that will be activated when clicked
...
if generate:
    ...
    <div class="card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
            ...
            <div style="background:var(--card-bg);color:var(--text-color);padding:4px 8px;border-radius:6px;font-weight:700">{dl}</div>
        </div>
    ...
    if show_barcodes:
        ...
        if _PDF417_AVAILABLE:
            try:
                ...
                svg_html = f"<div style='background:var(--card-bg);padding:8px;border-radius:6px;margin-top:12px;display:flex;justify-content:center'>{svg_str}</div>"
