...
def find_zips_by_city(city: str) -> List[str]:
    """Retourne tous les ZIP correspondant à une ville (insensible à la casse)."""
    target = (city or "").strip().lower()
    if not target:
        return []
    return [z for z, c in CALIFORNIA_ZIP_TO_CITY.items() if (c or "").strip().lower() == target]
